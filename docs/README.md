...
| Ignacio | |Los avances del proyecto fueron realizados en conjunto, cada uno aportó ideas creativas y útiles para el desarrollo del mismo, noté que realizar este trabajo en grupo fue, además de divertido, muy enriquescedor a nivel intelectual dado que aprendí muchas cosas útiles y que de verdad voy a poder aplicar en el porvenir de los días. Me permitió poner en práctica los conceptos que hemos ido aprendiendo durante lo que llevamos del semestre, ejemplo las técnicas de elicitación,los comandos de git, el modelo conceptual, casos de uso, user personas,user stories, requerimientos y sus respectivos tipos (Funcionales y No Funcionales), entre otras.
